...
@Slf4j
@Service
public class GrpcPojoClientService {

    private final HelloServiceGrpc.HelloServiceBlockingStub helloServiceBlockingStub;
    private final HelloServiceGrpc.HelloServiceStub helloServiceAsyncStub;

    /**
     * ManagedChannel을 주입받아 gRPC 서비스 스텁(Stub)들을 초기화합니다.
     * 이 채널은 GrpcChannelConfig에서 application.yml의 속성을 기반으로 생성됩니다.
     *
     * @param managedChannel 스프링 컨테이너에 의해 주입된 ManagedChannel 인스턴스
     */
    public GrpcPojoClientService(ManagedChannel managedChannel) {
        // 주입받은 ManagedChannel을 사용하여 동기 및 비동기 스텁을 생성합니다.
        this.helloServiceBlockingStub = HelloServiceGrpc.newBlockingStub(managedChannel);
        this.helloServiceAsyncStub = HelloServiceGrpc.newStub(managedChannel);
    }
    ...
    public String sendMessage(String requestMessage) {
        ...
        HelloResponse response = helloServiceBlockingStub.requestMessage(request);
